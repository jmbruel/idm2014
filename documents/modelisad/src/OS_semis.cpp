...
class OS_semis: public ve::fsa::Statechart {
public:
    enum State { IDLE, PROCESS };// liste des differents etat possibles de l'automate

    OS_semis(const vd::DynamicsInit& atom, const vd::InitEventList& events) :
        ve::fsa::Statechart(atom, events), signal("")
    {
        ...
        eventInState(this, "act", &OS_semis::act)
            >> IDLE;
        ...
        transition(this, IDLE, PROCESS)
            << guard(&OS_semis::signalRecu)
            << send(&OS_semis::out);

        transition(this, PROCESS, IDLE)
            << action(&OS_semis::clear);
...
private:
    ...
    void out(const vd::Time& time, vd::ExternalEventList& output) const
    {
    	if(signal == "stop"){
    		...
    		evt->putAttribute("value", new vv::Double(SEME));
...
DECLARE_DYNAMICS(modelisad::OS_semis)
