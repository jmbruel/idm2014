/*
 * @file ext/decision/KnowledgeBase.hpp
 *
...
#include <ext/decision/Library.hpp>
#include <ext/decision/Rules.hpp>
#include <ext/decision/Table.hpp>
...
public:
    ...
    void applyFact(const std::string& name, const vle::value::Value& value)
    { facts()[name](value); }
    ...
    vle::devs::Time duration(const vle::devs::Time& time);
     ...
     * ]-devs::negativeInfinity, vle::devs::infinity[ and a boolean if a change
     * has come.
     */
    Result processChanges(const vle::devs::Time& time);
     ...
     * @throw vle::utils::ArgError if activity is not in START state.
     */
     ...
     * @param date Date when activity failed.
     * @throw vle::utils::ArgError if activity is in FAILED state.
     */
