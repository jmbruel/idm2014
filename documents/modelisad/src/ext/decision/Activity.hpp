/*
 * @file ext/decision/Activity.hpp
 *
...
public:
    ...
    Activity()
        ...
        m_start(vle::devs::Time::negativeInfinity),
        m_finish(vle::devs::Time::infinity),
        m_minstart(vle::devs::Time::negativeInfinity),
        m_maxstart(vle::devs::Time::negativeInfinity),
        m_minfinish(vle::devs::Time::infinity),
        m_maxfinish(vle::devs::Time::infinity),
        m_started(vle::devs::Time::negativeInfinity),
        m_ff(vle::devs::Time::negativeInfinity),
        m_done(vle::devs::Time::negativeInfinity),
        mParams()
    {}
    ...
    void output(const std::string& name,
                vle::devs::ExternalEventList& events)
    ...
    void initStartTimeFinishTime(const vle::devs::Time& start,
                                 const vle::devs::Time& finish);

    void initStartTimeFinishRange(const vle::devs::Time& time,
                                  const vle::devs::Time& minfinish,
                                  const vle::devs::Time& maxfinish);

    void initStartRangeFinishTime(const vle::devs::Time& minstart,
                                  const vle::devs::Time& maxstart,
                                  const vle::devs::Time& time);

    void initStartRangeFinishRange(const vle::devs::Time& minstart,
                                   const vle::devs::Time& maxstart,
                                   const vle::devs::Time& minfinish,
                                   const vle::devs::Time& maxfinish);
     ...
     */
    bool isValidTimeConstraint(const vle::devs::Time& time) const;
    bool isBeforeTimeConstraint(const vle::devs::Time& time) const;
    bool isAfterTimeConstraint(const vle::devs::Time& time) const;
    ...
    void start(const vle::devs::Time& date) { m_state = STARTED; startedDate(date); }
    void ff(const vle::devs::Time& date) { m_state = FF; ffDate(date); }
    void end(const vle::devs::Time& date) { m_state = DONE; doneDate(date); }
    void fail(const vle::devs::Time& date) { m_state = FAILED; doneDate(date); }
    ...
    const vle::devs::Time& start() const { return m_start; }
    const vle::devs::Time& finish() const { return m_finish; }
    const vle::devs::Time& minstart() const { return m_minstart; }
    const vle::devs::Time& maxstart() const { return m_maxstart; }
    const vle::devs::Time& minfinish() const { return m_minfinish; }
    const vle::devs::Time& maxfinish() const { return m_maxfinish; }

    const vle::devs::Time& startedDate() const { return m_started; }
    const vle::devs::Time& doneDate() const { return m_done; }
    const vle::devs::Time& ffDate() const { return m_ff; }

    const vle::value::Map& parameters() const {return mParams;}
    vle::value::Map& parameters() {return mParams;}
     ...
     */
    vle::devs::Time nextTime(const vle::devs::Time& time);

private:
    void startedDate(const vle::devs::Time& date) { m_started = date; }
    void ffDate(const vle::devs::Time& date) { m_ff = date; }
    void doneDate(const vle::devs::Time& date) { m_done = date; }
    ...
    vle::devs::Time m_start;
    vle::devs::Time m_finish;
    vle::devs::Time m_minstart;
    vle::devs::Time m_maxstart;
    vle::devs::Time m_minfinish;
    vle::devs::Time m_maxfinish;

    vle::devs::Time m_started; /** Date when the activity is started. */
    vle::devs::Time m_ff; /**< Date when the activity is valid ff. */
    vle::devs::Time m_done; /** Date when the activity is done. */

    AckFct mAckFct;
    OutFct mOutFct;
    UpdateFct mUpdateFct;

    vle::value::Map mParams; /** Parameters of the activity **/
};
