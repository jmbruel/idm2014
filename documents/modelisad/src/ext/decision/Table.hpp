...
public:
     ...
     *
     * @throw vle::utils::ArgError if the key already exists.
     */
    void add(const std::string& name, const T& function)
    {
        ...
        if (not r.second) {
            throw vle::utils::ArgError(
                vle::fmt(_("Decision: id `%1%' already exists")) % name);
        }
    }
     ...
     *
     * @throw vle::utils::ArgError if the key does not exist.
     */
    ...
    iterator get(const std::string& name)
    {
        ...
        if (it == mLst.end()) {
            throw vle::utils::ArgError(
                vle::fmt(_("Decision: unknown id `%1%'")) % name);
        }
