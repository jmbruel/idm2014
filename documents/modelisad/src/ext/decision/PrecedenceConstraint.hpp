/*
 * @file ext/decision/PrecedenceConstraint.hpp
 *
...
public:
     ...
     * defines the result of the validity, the attribute vle::devs::Time the next
     * date to wake up the activity.
     */
    typedef std::pair < ResultType, vle::devs::Time > Result;
    ...
    const vle::devs::Time& mintimelag() const { return m_mintimelag; }
    const vle::devs::Time& maxtimelag() const { return m_maxtimelag; }

    iterator first() const { return m_first; }
    iterator second() const { return m_second ; }

    Result isValid(const vle::devs::Time& time) const;

private:
    Type       m_type;
    vle::devs::Time m_mintimelag;
    vle::devs::Time m_maxtimelag;
    iterator   m_first;
    iterator   m_second;
};
