...
Rule& Rules::add(const std::string& name, const Rule& rule)
{
    ...
    if (it != m_lst.end()) {
        throw vle::utils::ArgError(vle::fmt(_("Decision: rule '%1%' already exists"))
            % name);
    }
...
const Rule& Rules::get(const std::string& name) const
{
    ...
    if (it == m_lst.end()) {
        throw vle::utils::ArgError(vle::fmt(_("Decision: rule '%1%' does not exist"))
            % name);
    }
