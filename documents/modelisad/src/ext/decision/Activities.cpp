/*
 * @file ext/decision/Activities.cpp
 *
...
vle::devs::Time Activities::nextDate(const vle::devs::Time& time)
{
...
Activities::Result
Activities::process(const vle::devs::Time& time)
{
    vle::devs::Time nextDate = vle::devs::Time::infinity;
    Result update = std::make_pair(false, vle::devs::Time::infinity);
    ...
    do {
        ...
        for (iterator activity = begin(); activity != end(); ++activity) {
            ...
            default:
                throw vle::utils::InternalError(_("Decision: unknown state"));
            }
            ...
            if (update.second != time and
                update.second != vle::devs::Time::negativeInfinity) {
                nextDate = std::min(nextDate, update.second);
            }

            vle::devs::Time nextActivityDate = activity->second.nextTime(time);
            if (nextActivityDate != time and
                nextActivityDate != vle::devs::Time::negativeInfinity) {
                                                     ...
                                                     const vle::devs::Time& time)
{
    ...
    newstate.second = vle::devs::Time::infinity;
